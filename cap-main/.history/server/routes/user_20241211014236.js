...
const MailConfig = require("./config/mailConfig"); // Cấu hình email
const { hashValue } = require("./utils/hash"); // Đảm bảo có hàm hashValue
const ErrorHandler = require("./utils/ErrorHandler"); // Lỗi tùy chỉnh
...
router.post(`/changePassword`, async (req, res) => {
  ...
  if (!email) {
    return res.status(400).json({ error: true, msg: "Không tìm thấy email" });
  }
