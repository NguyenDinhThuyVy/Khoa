...
router.post("/buyProducts", async (req, res) => {
  try {
    const { order } = req.body; // Giả sử bạn gửi danh sách các sản phẩm trong giỏ hàng qua body
    const purchase = [];

    // Duyệt qua từng sản phẩm trong giỏ hàng
    for (const item of order) {
      const product = await Products.findById(item._id); // Tìm sản phẩm trong DB
      if (product) {
    ...
    const paymentData = {
      ...
      user: req.jwtDecoded.id, // Giả sử bạn có JWT cho người dùng
    };
